// ASTC constant tables. Generated by `python src/astc_ise.py --emit-glsl`, do not edit.
// Verified against the vendored astcenc sources by `python src/astc_ise.py`.
#ifndef ANBC_ASTC_TABLES_GLSL
#define ANBC_ASTC_TABLES_GLSL

// T byte of a 5-trit ISE block, indexed t4*81 + t3*27 + t2*9 + t1*3 + t0.
const uint kAstcIntegerOfTrits[243] = uint[243](
...
const uint kAstcUnquant48[48] = uint[48](
...
const uint kAstcCodeOfValue48[256] = uint[256](
...
const uint kAstcUnquant192[192] = uint[192](
...
const uint kAstcCodeOfValue192[256] = uint[256](
...
const float kAstcWeights4[4] = float[4](0.0 / 64.0, 21.0 / 64.0, 43.0 / 64.0, 64.0 / 64.0);
// QUANT_8 weights (unquantized 0..64, as blend factors).
const float kAstcWeights8[8] = float[8](0.0 / 64.0, 9.0 / 64.0, 18.0 / 64.0, 27.0 / 64.0, 37.0 / 64.0, 46.0 / 64.0, 55.0 / 64.0, 64.0 / 64.0);
// QUANT_16 weights (unquantized 0..64, as blend factors).
const float kAstcWeights16[16] = float[16](0.0 / 64.0, 4.0 / 64.0, 8.0 / 64.0, 12.0 / 64.0, 17.0 / 64.0, 21.0 / 64.0, 25.0 / 64.0, 29.0 / 64.0, 35.0 / 64.0, 39.0 / 64.0, 43.0 / 64.0, 47.0 / 64.0, 52.0 / 64.0, 56.0 / 64.0, 60.0 / 64.0, 64.0 / 64.0);

#endif // ANBC_ASTC_TABLES_GLSL
