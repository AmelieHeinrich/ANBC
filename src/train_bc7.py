"""Train a small MLP to predict BC7 blocks from raw 4x4 blocks.
...
BC5, BC6H and ASTC 4x4 ship without a network: their MLPs did not beat the
block min/max init after refinement (see notes.txt), so those encoders are
purely analytic (bc5_codec.py / bc6h_codec.py / astc_codec.py).
...
def get_dataset_files(mode: str, split: str, limit_images: int | None, val_images: int) -> list[Path]:
    ...
    # mode5: flat local folder, the last `val_images` files held out.
    files = sorted(p for p in AMBIENTCG_ALPHA_DIR.iterdir() if p.suffix.lower() in IMAGE_EXTS)
    if len(files) <= val_images:
        raise ValueError(f"only {len(files)} images in {AMBIENTCG_ALPHA_DIR}, need more than val_images={val_images}")
...
def build_block_dataset(
    ...
    (N, 64) uint8 (RGBA channel order). Kept as integers (not float32) so
    the full DIV2K block set (~35GB as float32) fits comfortably in RAM
    ...
    *chunk order* and slicing contiguously, instead of doing a full random
    ...
    for f in tqdm(files, desc=f"loading {mode} {split} blocks"):
        blocks = extract_blocks(load_rgba(f))  # (N, 4, 4, 4)
...
def run_model(model: nn.Module, mode: str, batch01: torch.Tensor) -> torch.Tensor:
    """Run the model on a (B, 64) float batch in [0,1] and return the
    reconstructed (B, 16, 4) pixels via the appropriate soft-decode."""
    if mode == "mode6":
        endpoint0, endpoint1, interp = model(batch01)
...
def train(
    ...
    if mode == "mode6":
        model = BC7Mode6MLP(hidden_dim=hidden_dim).to(device_t)
    else:
    ...
    for epoch in range(epochs):
        ...
        for chunk_idx in pbar:
            ...
            recon = run_model(model, mode, batch)
        ...
        val_metrics = evaluate(model, mode, val_blocks, device_t)
        print(
        ...
        ckpt_path = CHECKPOINT_DIR / f"bc7_{mode}_mlp.pt"
        torch.save(
            {"model_state": model.state_dict(), "mode": mode, "arch": "mlp", "hidden_dim": hidden_dim},
            ckpt_path,
        )
...
@torch.no_grad()
def evaluate(model: nn.Module, mode: str, val_blocks: torch.Tensor, device_t: torch.device, chunk: int = 65536) -> dict:
    """Validation metrics. Runs in chunks so a large validation set doesn't
    have to fit on the GPU as one float32 batch plus its reconstruction."""
    model.eval()
    ...
    flip_score = compute_flip_on_subset(first_target, first_recon)
...
def compute_flip_on_subset(target: torch.Tensor, recon: torch.Tensor, num_blocks: int = 64) -> float:
    ...
    def blocks_to_image(x: torch.Tensor) -> np.ndarray:
        ...
        return np.clip(arr, 0.0, 1.0).astype(np.float32)[..., :3]
...
if __name__ == "__main__":
    ...
    # The local-folder dataset (mode5) is small; cap the hold-out.
